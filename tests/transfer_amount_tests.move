...
fun test_transfer_amount_partial_balance() {
    ...
    let remaining_coin = ts::take_from_address<Coin<RDS>>(&scenario, sender);
    assert!(coin::value(&remaining_coin) == 400, 1);
    
    ts::return_to_address(recipient, received_coin);
    ts::return_to_address(sender, remaining_coin);
    ts::end(scenario);
}
...
fun test_transfer_amount_large_amounts() {
    ...
    let received_coin = ts::take_from_address<Coin<RDS>>(&scenario, recipient);
    assert!(coin::value(&received_coin) == large_amount, 0);
    
    // Verify: sender received remainder
    let remaining_coin = ts::take_from_address<Coin<RDS>>(&scenario, sender);
    assert!(coin::value(&remaining_coin) == 100000000000000 - large_amount, 1);
...
fun test_transfer_amount_multiple_sequential_transfers() {
    ...
    let coin1 = ts::take_from_address<Coin<RDS>>(&scenario, recipient1);
    let coin2 = ts::take_from_address<Coin<RDS>>(&scenario, recipient2);
    let final_remaining = ts::take_from_address<Coin<RDS>>(&scenario, sender);
    
    assert!(coin::value(&coin1) == 300, 0);
    assert!(coin::value(&coin2) == 200, 1);
    assert!(coin::value(&final_remaining) == 500, 2); // 1000 - 300 - 200
    
    ts::return_to_address(recipient1, coin1);
    ts::return_to_address(recipient2, coin2);
    ts::return_to_address(sender, final_remaining);
    ts::end(scenario);
}
